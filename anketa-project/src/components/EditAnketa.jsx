...
const EditAnketa = observer(() => {
  const { Product } = useStores();
  const [open, setOpen] = React.useState(false);
  const handleOpen = () => setOpen(true);
  const handleClose = () => setOpen(false);

  const initialData = Product.user_data?.details || {
    fullName: "",
    birthDate: "",
    nationality: "",
    citizenship: "",
  ...
  const handleSave = () => {
    ...
    Product.updateAnketa(formData); // если есть метод обновления в store
    setOpen(false);
  };
  ...
  return (
    <>
      ...
      >
        <Box sx={style}>
          <Typography variant="h6">Редактирование анкеты</Typography>
          <Box
            component="form"
